...
fn on_exit_data(context: &mut CompileContext) -> Result<(), message::Message> {
    ...
    if let Node::Text(text) = context.tail_mut() {
        // WRITR-RS PATCH: a token that starts in the middle of a tab carries
        // leading virtual spaces, which micromark's `sliceSerialize`
        // materializes as real spaces (e.g. indented code such as
        // `- foo\n\n\t\tbar` must yield `  bar`). `as_str()` drops them;
        // `to_html.rs` handles them via `Slice::serialize`. Trailing virtual
        // spaces stay dropped, matching `sliceSerialize` without
        // `expandTabs` (they follow the tab character inside the slice).
        for _ in 0..value.before {
            text.value.push(' ');
        }
